...
pub(crate) fn estimate_inter_costs_histogram_blocks<T: Pixel>(
  ...
  (0..h_in_imp_b).for_each(|y| {
    (0..w_in_imp_b).for_each(|x| {
      ...
      let mut histogram_org = Histogram::configure()
        .max_value(256)
        .build()
        .unwrap();
      ...
      let mut histogram_ref = Histogram::configure()
        .max_value(256)
        .build()
        .unwrap();
      ...
      let mean = (histogram_org.mean().unwrap() as i32  - histogram_ref.mean().unwrap() as i32).abs();
